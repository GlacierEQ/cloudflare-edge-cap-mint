...
**Repository:** `GlacierEQ/cloudflare-edge-cap-mint`  
**Company lens (independent):** Cloudflare (`cloudflare`)  
**Innovation:** Edge Cap Mint  
**Scaffold batch:** 2026-08-10T0924Z

## Mission

Implement a **real, testable** central mechanism that addresses the bottleneck below. Do **not** claim Cloudflare affiliation, proprietary access, or production deployment.

### Bottleneck
Durable low-latency agent runtime and secure state across the edge.

### Brick wall
Isolating long-running agents and credentials while maintaining global availability, cost, and content and tool safety.

### Mechanism to implement
Mint path-bound, TTL-limited edge capabilities with explicit revoke and scope receipts.

## Hard rules (fail closed)

1. **No affiliation theater** — never state or imply Cloudflare employment, endorsement, or proprietary systems access.
...
File: `src/edge_cap_mint.py`

- Expand `EdgeCapMint` into a complete, self-contained implementation.
- Public API must stay stable enough that tests in `tests/test_edge_cap_mint.py` can be upgraded (not gutted).
- Include at least:
...
### 3. Strengthen tests
Files: `tests/test_edge_cap_mint.py`, `tests/test_adversarial.py`
...
- Cloud deploy, customer pilots, proprietary Cloudflare APIs
